...
class NetworkFactory(object):
    ...
    def resume_train(self, ckpt, resume):
        ...
        if resume:
            ...
            start_iter = "".join(list(filter(str.isdigit, ckpt)))
    ...
    def warm_cos_lr(self, lr, iters):
        if iters <= system_configs.warmup_total_iters:
            lr = 100 * system_configs.warmup_lr_start * iters / float(
                system_configs.warmup_total_iters
            ) + system_configs.warmup_lr_start
        else:
            lr = system_configs.warmup_lr_start + 5 * system_configs.warmup_lr_start * \
                 (1.0 + math.cos(math.pi * (iters - system_configs.warmup_total_iters) / (system_configs.period)))
        return lr